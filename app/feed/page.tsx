...
import PostCard from '@/components/Postcard'; // <-- ensure filename matches
import { prisma } from '@/lib/prisma';
import { getServerSession } from 'next-auth/next';
import { authConfig } from '@/lib/auth';
...
export default async function FeedPage() {
  ...
  // 2) Enemy posts (exclude yourself)  ✅ fixed filter
  const enemyPosts = await prisma.post.findMany({
    where: {
      authorId: { in: enemyIds },
      NOT: { authorId: userId },
    },
    include: postInclude,
    orderBy: { createdAt: 'desc' },
  ...
  const otherPosts = await prisma.post.findMany({
    where: {
      AND: [{ authorId: { notIn: enemyIds } }, { authorId: { not: userId } }],
    },
    include: postInclude,
    orderBy: { createdAt: 'desc' },
    take: 20,
  ...
  // 4) Shape with counts (+ optional userReaction + replies)
  const mapPost = (p: (typeof enemyPosts)[number]) => {
    const tallies: Record<'SHADE' | 'BOO' | 'MUM', number> = {
      SHADE: 0,
      BOO: 0,
      MUM: 0,
    };
    ...
    return {
      ...
      counts: tallies,
      userReaction,
      // Pass replies so PostCard -> CommentBox can render them
